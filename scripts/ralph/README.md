...
cp /path/to/scripts/ralph/ralph.sh scripts/ralph/
cp /path/to/scripts/ralph/prompt.md scripts/ralph/
chmod +x scripts/ralph/ralph.sh
```

### Option 2: Use directly from this folder

Ralph is already set up in this project's `scripts/ralph/` directory. Just run:
...
Create or update `~/.claude/settings.json` with your preferences:
...
Or use the `--dangerously-skip-permissions` flag (already included in ralph.sh) for autonomous operation.

## Workflow

### 1. Create a PRD

Create a detailed requirements document. You can ask Claude to help:

```
Create a PRD for [your feature description] following the format in scripts/ralph/skills/prd/SKILL.md
```
...
```
Convert tasks/prd-[feature-name].md to scripts/ralph/prd.json following the format in scripts/ralph/skills/ralph/SKILL.md
```
...
| `skills/prd/` | Guide for generating PRDs |
| `skills/ralph/` | Guide for converting PRDs to JSON |
